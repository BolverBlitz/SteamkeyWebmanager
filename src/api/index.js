...
fs.readdir("./src/api", function(err, filenames) {
  ...
  for (i = 0; i < filenames.length; i++) { 
    if(filenames[i].startsWith("disabled.")){
      logger('warning',`SKW: Skipped API Plugin ${filenames[i].slice(9, filenames[i].length-3)} because its disabled`);
    }else if(filenames[i].endsWith(".js")){
      ...
      if(typeof(iimport.PluginRequirements) === "undefined" || typeof(iimport.PluginName) === "undefined" || typeof(iimport.PluginVersion) === "undefined" || typeof(iimport.PluginAuthor) === "undefined" || typeof(iimport.PluginDocs) === "undefined" || typeof(iimport.router) === "undefined"){
        logger('error',`SKW: Skipped API Plugin ${filenames[i].slice(0, filenames[i].length-3)} because there are missing exports!`);
      }else{
        ...
        if(!PluginRequirementsFailed){
          ...
          logger('system',`SKW: Loaded API Plugin ${filenames[i].slice(0, filenames[i].length-3)}`);
        }else{
          logger('error',`SKW: API Plugin ${filenames[i].slice(0, filenames[i].length-3)}} requires following plugins [${iimport.PluginRequirements}] and at least one is missing!`);
        }
      }
    }else{
      logger('error',`SKW: Unknown file was skipped ${filenames[i]}`);
    }
  }
})
