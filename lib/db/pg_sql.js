...
const WriteNewKey = function (id, key, name, status = 1, owner) {
    return new Promise(function (resolve, reject) {
        pool.query('INSERT INTO keyslist (id, key, name, status, owner) VALUES ($1,$2,$3,$4,$5)', [
            id, key, name, status, owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
        });
...
const GetAllKeysBasedOnStatusWithKeyFailandSuccsessJoinToUserTable = function (status) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.username FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.status = $1`, [
            status
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
