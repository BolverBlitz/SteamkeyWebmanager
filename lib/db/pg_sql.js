...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTable = function () {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE NOT keyslist.status = 4 ORDER BY name ASC`,
            (err, result) => {
                if (err) { reject(err) }
                resolve(result);
            });
...
const GetAllKeysBasedOnStatusWithKeyFailandSuccsessJoinToUserTable = function (status) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.status = $1 ORDER BY name ASC`, [
            status
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTableWithOwner = function (owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.owner = $1 AND NOT keyslist.status = 4 ORDER BY name ASC`, [
            owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetAllKeysBasedOnStatusWithKeyFailandSuccsessJoinToUserTableWithOwner = function (status, owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.status = $1 AND keyslist.owner = $2 ORDER BY name ASC`, [
            status, owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
