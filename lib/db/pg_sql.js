...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTable = function () {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username`,
            (err, result) => {
                if (err) { reject(err) }
                resolve(result);
            });
...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTableWithOwner = function (owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.owner = $1`, [
            owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
 ...
 * @returns {Promise}
 */
const UpdateKeyGifted = function (id, gifted_to) {
    return new Promise(function (resolve, reject) {
