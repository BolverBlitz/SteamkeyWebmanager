...
pool.query(`CREATE TABLE IF NOT EXISTS users (
    ...
    keys_success bigint DEFAULT 0,
    keys_failed bigint DEFAULT 0,
    lang text,
    time TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
 ...
 * @param {Number} [status]
 * @param {String} owner 
 * @param {String} giftaccess
 * @returns {Promise}
 */
const WriteNewKey = function (id, key, name, status = 1, owner, giftaccess) {
    return new Promise(function (resolve, reject) {
        pool.query('INSERT INTO keyslist (id, key, name, status, status_modifyed, owner, giftaccess, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [
            id, key, name, status, new Date(), owner, giftaccess, new Date()
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
        });
...
const GetAllKeys = function () {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT id, name, status, giftaccess, status_modifyed, created_at FROM keyslist`, (err, result) => {
            if (err) { reject(err) }
            resolve(result);
        });
    });
...
/**
 * This function will return all encrypted keys from DB
 * @returns {Promise}
 */
 const GetAllEncryptedKeys = function () {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT key FROM keyslist`, (err, result) => {
            if (err) { reject(err) }
            resolve(result);
        });
    });
...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTable = function () {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE NOT keyslist.status = 4`,
            (err, result) => {
                if (err) { reject(err) }
                resolve(result);
            });
...
const GetAllKeysBasedOnStatusWithKeyFailandSuccsessJoinToUserTable = function (status) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.status = $1`, [
            status
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetAllKeysWithKeyFailandSuccsessJoinToUserTableWithOwner = function (owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.owner = $1 AND NOT keyslist.status = 4`, [
            owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetAllKeysBasedOnStatusWithKeyFailandSuccsessJoinToUserTableWithOwner = function (status, owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.giftaccess, keyslist.status_modifyed, keyslist.owner, keyslist.created_at, users.keys_success, users.keys_failed FROM keyslist LEFT JOIN users ON keyslist.owner = users.username WHERE keyslist.status = $1 AND keyslist.owner = $2`, [
            status, owner
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetAllKeysBasedOnStatus = function (status) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT id, name, status, giftaccess, status_modifyed, created_at FROM keyslist WHERE status = $1`, [
            status
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const GetKeyDetailsSave = function (id) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT id, name, status, owner, giftaccess, status_modifyed, created_at, gifted_to, gifted_at FROM keyslist WHERE id = $1`, [
            id
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const UpdateKeyGiftedto = function (id, gifted_to) {
    return new Promise(function (resolve, reject) {
        pool.query(`UPDATE keyslist SET status = 4, gifted_to = $1, gifted_at = CURRENT_TIMESTAMP WHERE id = $2`, [
            gifted_to, id
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const UpdateKeyGiftedStatus = function (id) {
    return new Promise(function (resolve, reject) {
        pool.query(`UPDATE keyslist SET status = 3, status_modifyed = CURRENT_TIMESTAMP WHERE id = $1`, [
            id
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
/**
 * Check if a vote is actualy a valid vote request
 * @param {String} id 
 * @param {String} owner 
 * @returns 
 */
const CheckIfKeyIsGiftedButNotVoted = function (id, owner) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT * FROM keyslist WHERE id = $1 AND owner = $2 AND status = 4 AND voted = 0`, [
            id, owner
        ], (err, result) => {
            if (err) { reject(err) }
            if (result.rows.length > 0) {
...
/**
 * Set voted status for a key, 0=NoVote, 1=valid, 2=invalid
 * @param {String} id 
 * @param {Number} Voted 
 * @returns 
 */
const SetKeyVoted = function (id, Voted) {
    return new Promise(function (resolve, reject) {
        pool.query(`UPDATE keyslist SET voted = $1 WHERE id = $2`, [
            Voted, id
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const IncreeseUserKeySuccess = function (username) {
    return new Promise(function (resolve, reject) {
        pool.query(`UPDATE users SET keys_success = keys_success + 1 WHERE username = $1`, [
            username
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result)
...
const IncreeseUserKeyFail = function (username) {
    return new Promise(function (resolve, reject) {
        pool.query(`UPDATE users SET keys_failed = keys_failed + 1 WHERE username = $1`, [
            username
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result)
...
/**
 * Get all gifts that are from one owner
 * @param {String} owner 
 * @returns 
 */
...
/**
 * Get Keylist data from a linked gift using url_token
 * @param {String} url_token 
 * @returns 
 */
const GetKeyDetailsSaveFromGift = function (url_token) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.id, keyslist.name, keyslist.status, keyslist.status_modifyed, keyslist.owner, keyslist.created_at FROM keyslist INNER JOIN gifts ON keyslist.id = gifts.id WHERE gifts.url_token = $1`, [
            url_token
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
/**
 * Get the actual key from url_token
 * @param {String} url_token 
 * @returns 
 */
const GetKeyFromGift = function (url_token) {
    return new Promise(function (resolve, reject) {
        pool.query(`SELECT keyslist.key FROM keyslist INNER JOIN gifts ON keyslist.id = gifts.id WHERE gifts.url_token = $1`, [
            url_token
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const DelGiftByToken = function (url_token) {
    return new Promise(function (resolve, reject) {
        pool.query(`DELETE FROM gifts WHERE url_token = $1`, [
            url_token
        ], (err, result) => {
            if (err) { reject(err) }
            resolve(result);
...
const key = {
    ...
    read: {
        ...
        check: {
            vote: CheckIfKeyIsGiftedButNotVoted,
            key: CheckKeyDetails,
        }
    },
    delete: {
...
const gift = {
    ...
    read: {
        ...
        keysave: GetKeyDetailsSaveFromGift,
        key: GetKeyFromGift
    },
    delete: {
        gift: DelGiftByToken
